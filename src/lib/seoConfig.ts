import { BUSINESS_NAME, PHONE_1, PHONE_2, LOCATION, ADDRESS } from "./constants";

export const SITE_URL = "https://shivasakthiinvisiblegrills.com";

// Service Areas for Local SEO
export const SERVICE_AREAS = [
  { name: "Vijayawada", state: "Andhra Pradesh", postalCode: "520010" },
  { name: "Guntur", state: "Andhra Pradesh", postalCode: "522001" },
  { name: "Machilipatnam", state: "Andhra Pradesh", postalCode: "521001" },
  { name: "Tenali", state: "Andhra Pradesh", postalCode: "522201" },
  { name: "Gudivada", state: "Andhra Pradesh", postalCode: "521301" },
  { name: "Mangalagiri", state: "Andhra Pradesh", postalCode: "522503" },
];

// Reviews Data (for schema)
export const BUSINESS_REVIEWS = [
  {
    author: "Rajesh Kumar",
    rating: 5,
    datePublished: "2024-01-15",
    reviewBody: "Excellent work! The invisible grills look amazing and my kids are safe now. Highly recommended!",
  },
  {
    author: "Priya Sharma",
    rating: 5,
    datePublished: "2024-02-10",
    reviewBody: "Very professional installation. The grills are completely invisible and the quality is outstanding.",
  },
  {
    author: "Venkat Rao",
    rating: 5,
    datePublished: "2024-03-05",
    reviewBody: "Best invisible grill service in Vijayawada. Fair pricing and excellent workmanship. 5 stars!",
  },
];

// Long-tail Keywords by Service
export const SEO_KEYWORDS = {
  balcony: [
    "invisible grills for balcony vijayawada",
    "balcony safety grills vijayawada",
    "child safety balcony grills vijayawada",
    "pigeon protection balcony grills",
    "invisible grills installation near me",
    "balcony safety net vijayawada",
  ],
  window: [
    "window invisible grills vijayawada",
    "window safety grills vijayawada",
    "bird protection window grills",
    "invisible grills for windows",
    "window safety net vijayawada",
  ],
  child: [
    "child safety grills vijayawada",
    "kid safety invisible grills",
    "children safety net for balcony",
    "child proof balcony grills",
    "invisible grills for child safety",
  ],
  commercial: [
    "commercial invisible grills vijayawada",
    "office building safety grills",
    "high rise building grills",
    "apartment invisible grills",
  ],
};

// Location-specific keywords
export const LOCATION_KEYWORDS = {
  vijayawada: "invisible grills vijayawada, safety grills vijayawada, balcony grills vijayawada",
  guntur: "invisible grills guntur, safety grills guntur, balcony grills guntur",
  tenali: "invisible grills tenali, safety grills tenali",
  mangalagiri: "invisible grills mangalagiri, safety grills mangalagiri",
};

// Social Media Profiles
export const SOCIAL_PROFILES = [
  "https://www.facebook.com/shivasakthi-grills",
  "https://www.instagram.com/shivasakthi-grills",
  "https://twitter.com/shivasakthi_grills",
];

// Payment Methods Accepted
export const PAYMENT_METHODS = [
  "Cash",
  "UPI",
  "Bank Transfer",
  "Debit Card",
  "Credit Card",
];

// Certifications & Trust Signals
export const CERTIFICATIONS = [
  "ISO Certified Materials",
  "BIS Approved Cables",
  "Insured Installation Service",
  "5 Year Warranty",
];
